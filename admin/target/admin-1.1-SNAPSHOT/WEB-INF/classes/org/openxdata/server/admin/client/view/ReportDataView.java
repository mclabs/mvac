...
/**
 * This widget displays data in a report and also allows you export it to PDF format.
 * 
 * @author daniel
 *
 */
public class ReportDataView  extends Composite{
	...
	/** Reference to the report properties view. For now the only use of this is to
	 *  to get us the report definition sql.
	 */
	 ...
	 * 
	 * @param rptView a reference to report properties view
	 */
	...
	private void loadReportData(){
		...
		DeferredCommand.addCommand(new Command(){
			...
			public void execute() {
				try{
					Context.getReportService().getReportData(report,"html", new OpenXDataAsyncCallback<String>() {
						@Override
	...
	private void exportPdf(){
		...
		DeferredCommand.addCommand(new Command(){
			...
			public void execute() {
				try{
					reportWidget.setHTML("<object type=\"application/pdf\" data=\"report?format=pdf&name=rates.pdf&reportId="+report.getReportId()+"\" width=\"500\" height=\"650\" > </object>");
					FormUtil.dlg.hide();
				}
				catch(Exception ex){
					FormUtil.dlg.hide();
