...
public class StudiesTreeViewHelper {
	...
	public static void exportSelectedItem(TreeItem item, GetFileNameDialogEventListener eventListener){
		...
		name = name.replace(" ", "");
		new GetFileNameDialog(eventListener, OpenXDataFacade.getOpenXDataConstants().label_export_as(),OpenXDataFacade.getOpenXDataConstants().label_export(),name).center();
	}
	...
	public static void addNewItem(String xForm, Tree tree, List<StudyDef> studies, PopupPanel popup){
		...
		if(item == null || item.getUserObject() instanceof StudyDef){
			...
			TreeItem root = new CompositeTreeItem(new TreeItemWidget(widgetFactory.getOpenXDataImages().note(), studyDef.getName(),popup));
			...
			item = addImageItem(parent, formDef.getName(), widgetFactory.getOpenXDataImages().drafts(), formDef, null, popup);
			...
			item = addImageItem(parent, formDefVersion.getName(), widgetFactory.getOpenXDataImages().markRead(),formDefVersion, null, popup);
	...
	public static void addNewChildItem(Tree tree, List<StudyDef> studies, PopupPanel popup ) {
		...
		if(item.getUserObject() instanceof StudyDef){
			...
			item = addImageItem(item, formDef.getName(), widgetFactory.getOpenXDataImages().drafts(), formDef, null, popup);
			...
			item = addImageItem(item, formDefVersion.getName(), widgetFactory.getOpenXDataImages().markRead(), formDefVersion, null, popup);
	...
	public static void importForm(Tree tree, TreeItem parent, FormDef formDef, PopupPanel popup){
		...
		TreeItem item = addImageItem(parent, formDef.getName(), widgetFactory.getOpenXDataImages().drafts(), formDef, null, popup);
		tree.setSelectedItem(item);
		item.getParentItem().setState(true);
	...
	public static void importFormVersion(Tree tree, TreeItem parent, FormDefVersion formDefVersion, PopupPanel popup) {
		...
		TreeItem item = addImageItem(parent, formDefVersion.getName(), widgetFactory.getOpenXDataImages().markRead(), formDefVersion, null, popup);
		tree.setSelectedItem(item);
		item.getParentItem().setState(true);
	 ...
	 * @param itemSelectionListener <tt>ItemSelectionListener</tt> for the data check operation.
	 * @param popup <tt>Popup</tt> for the <tt>Tree View.</tt>
	 */
	public static void onOptionSelected(int option, Tree tree, TreeItem item, ItemSelectionListener itemSelectionListener, PopupPanel popup){
		if(option == 0){
			...
			((StudyView)itemSelectionListener).designItem(true, tree.getSelectedItem().getUserObject());
		}
		else if(option == 2){
			return;
		}
	...
	public static void changeEditableProperties(Object item, Tree tree, PopupPanel popup){
		...
		if(item instanceof StudyDef){
			...
			treeItem.setWidget(new TreeItemWidget(widgetFactory.getOpenXDataImages().note(), studyDef.getName(),popup));
			treeItem.setTitle(studyDef.getDescription());
			studyDef.setDirty(true);
		}
		else if(item instanceof FormDef){
			FormDef formDef = (FormDef)item;
			treeItem.setWidget(new TreeItemWidget(widgetFactory.getOpenXDataImages().drafts(), formDef.getName(),popup));
			...
			treeItem.setWidget(new TreeItemWidget(widgetFactory.getOpenXDataImages().markRead(), formDefVersion.getName(),popup));
	...
	public static void loadStudy(StudyDef studyDef){
		...
		TreeItem studyRoot = new CompositeTreeItem(new TreeItemWidget(widgetFactory.getOpenXDataImages().note(), studyDef.getName(),popup));
	...
	private static void loadForm(FormDef formDef, TreeItem studyRoot, boolean select){
		...
		TreeItem formRoot = StudiesTreeViewHelper.addImageItem(studyRoot, formDef.getName(), widgetFactory.getOpenXDataImages().drafts(), formDef, "", popup);
		
		List<FormDefVersion> versions = formDef.getVersions();
		if(versions != null){
			for(FormDefVersion versionDef : versions){
				addImageItem(formRoot, versionDef.getName(), widgetFactory.getOpenXDataImages().markRead(),versionDef, null, popup);
			}
		}
	...
	private static TreeItem constructTreeItem(StudyDef xDef) {
		TreeItem root = new CompositeTreeItem(new TreeItemWidget(widgetFactory.getOpenXDataImages().note(), xDef.getName(), popup));		
