...
public class ReportsViewController extends OpenXDataObservable implements SaveCompleteListener {
	 ...
	 */
	public ReportsViewController(OpenXDataBaseTreeView reportsTreeView, OpenXDataBaseView reportView, OpenXDataBaseView userView) {
		
		this.userView = (UserView) userView;
		this.reportView = (ReportView) reportView;
		this.reportsTreeView = (ReportsTreeView) reportsTreeView;
	}
	 ...
	 * database
	 */
	public void saveMappedReportGroups() {
		if(userMappedReportGroups == null)
			return;
		
		List<UserReportGroupMap> deletedMappedReportGroups = (List<UserReportGroupMap>) userView.getDeletedUserMappedReportGroups();
		
		SaveAsyncCallback callback = new SaveAsyncCallback(
				MainViewControllerUtil.getDirtyCount(userMappedReportGroups) + 
				(deletedMappedReportGroups != null ? deletedMappedReportGroups.size() : 0),
				OpenXdataText.get(TextConstants.MAPPED_REPORTS_SAVED_SUCCESSFULLY),OpenXdataText.get(TextConstants.PROBLEM_SAVING_MAPPED_REPORTS),reportGroups,
				deletedMappedReportGroups,this);

		for(UserReportGroupMap map : userMappedReportGroups){
			if(!map.isDirty())
				continue;
			else{
				callback.setCurrentItem(map);
			...
			Context.getReportService().saveUserMappedReportGroup(map, callback);
		}
		
		if(deletedMappedReportGroups != null){
			for(UserReportGroupMap map : deletedMappedReportGroups){
				callback.setCurrentItem(map);
				Context.getReportService().deleteUserMappedReportGroup(map, callback);
			}
		}
