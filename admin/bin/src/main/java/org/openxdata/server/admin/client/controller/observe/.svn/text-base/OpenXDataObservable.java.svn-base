...
public class OpenXDataObservable {
	...
	public void notifyObservers(Object arg, Class<?> typeClass) {
		...
		for(OpenXDataObserver xObserver : observers){
			...
			//Notifying RolesViewController observers
			if(xObserver instanceof RolesObserver){
				updateRolesObserverObjects(arg, typeClass, xObserver);
			}
	...
	private synchronized void updateUtilityObserverObjects(Object notifiedObjects, Class<?> typeClass, OpenXDataObserver xObserver) {
		if(typeClass.equals(Locale.class)){
			...
			if(typeClass.equals(MobileMenuText.class)){
				((UtilityObserver)xObserver).updateMobileMenuText(this, (List<MobileMenuText>) notifiedObjects);
			}
	}

	/**
	 * Updates all Observers listening for notification on the <code>RolesViewController.</code>
	 * 
	 * @param notifiedObjects The model objects that have changed.
	 * @param typeClass The class of the model object.
	 * @param xObserver <code>Observer</code> that was registered to listen for notifications.
	 */
	@SuppressWarnings("unchecked")
	private synchronized void updateRolesObserverObjects(Object notifiedObjects, Class<?> typeClass, OpenXDataObserver xObserver) {
		if(typeClass.equals(Role.class)){
			((RolesObserver)xObserver).updateRoles(this, (List<Role>) notifiedObjects);
		}
		else 
			if(typeClass.equals(Permission.class)){
			((RolesObserver)xObserver).updatePermissions(this, (List<Permission>) notifiedObjects);
		}
	}
