 ...
 *
 */
public class SettingsTreeView extends OpenXDataBaseTreeView implements ExtendedContextInitMenuListener, OpenXDataViewApplicationEventListener, SettingsObserver {
	 ...
	 * @param mainView 
	 */
	public SettingsTreeView(OpenXDataViewFactory openXDataViewFactory){		
	...
	public void deleteSelectedItem(){		
		...
		if(item == null){
			Window.alert(OpenXDataFacade.getOpenXDataConstants().label_deletesettingselect());
			return;
		}

		//TODO add message for internationalization purposes
		if( !Window.confirm(OpenXDataFacade.getOpenXDataConstants().label_deletesettingconfirmation()))
			return;
	 ...
	 * 
	 * @param settingGroups the list of setting groups.
	 */
	public void loadSettingGroups(List<SettingGroup> settingGroups){		
		...
		if(isLoadData()){
			tree.clear();
			if(RolesListUtil.getPermissionResolver().isPermission("SettingGroup")){
				TreeItem root; SettingGroup settingGroup;
				for(int i=0; i<settingGroups.size(); i++){
					settingGroup = settingGroups.get(i);
					if(settingGroup.getParentSettingGroup() == null){
						root = new CompositeTreeItem(new TreeItemWidget(widgetFactory.getOpenXDataImages().lookup(), settingGroup.getName(),popup));
	...
	private void loadSettingGroup(SettingGroup settingGroup,TreeItem parent){
		if(settingGroup.getSettings() != null){
			for(Setting setting : settingGroup.getSettings()){
				TreeItem item = new CompositeTreeItem(new TreeItemWidget(widgetFactory.getOpenXDataImages().filtersgroup(), setting.getName(),popup));;
		...
		if(settingGroup.getGroups() != null){
			for(SettingGroup stgGroup : settingGroup.getGroups()){
				TreeItem item = new CompositeTreeItem(new TreeItemWidget(widgetFactory.getOpenXDataImages().lookup(), stgGroup.getName(),popup));;
	...
	public void addNewSetting(){		
		...
		TreeItem root = new CompositeTreeItem(new TreeItemWidget(widgetFactory.getOpenXDataImages().filtersgroup(), setting.getName(),popup));
	...
	public void addNewSettingGroup(){
		...
		TreeItem root = new CompositeTreeItem(new TreeItemWidget(widgetFactory.getOpenXDataImages().lookup(), settingGroup.getName(),popup));
		root.setUserObject(settingGroup);
		settingGroup.setDirty(true);
	...
	/**
	 * Adds a new setting group as a child of the selected item
	 * which should be another setting group.
	 */
	@Override
	public void addNewItem(){
		...
		if(parent != null){
			parent.addItem(root);
			settingGroup.setParentSettingGroup((SettingGroup)parent.getUserObject());

			if(!settingGroup.getParentSettingGroup().isNew())
				settingGroups.add(settingGroup);
			else
				settingGroup.getParentSettingGroup().addSettingGroup(settingGroup);
	...
	public void changeEditableProperties(Object item) {		
		...
		if(item instanceof Setting){
			...
			treeItem.setWidget(new TreeItemWidget(widgetFactory.getOpenXDataImages().filtersgroup(), setting.getName(),popup));
			treeItem.setTitle(setting.getDescription());
			setting.setDirty(true);
		}
		else{
			SettingGroup settingGroup = (SettingGroup)item;
			treeItem.setWidget(new TreeItemWidget(widgetFactory.getOpenXDataImages().lookup(), settingGroup.getName(),popup));
	...
	public boolean isValidSettingsList()
	{
		...
		for(int j=0;j<index;j++)
		{
			if(map.containsKey(tree.getItem(j).getText().toLowerCase())){
				...
				Window.alert(OpenXDataFacade.getOpenXDataConstants().label_existingsetting()+tree.getItem(j).getText());
			    return false;
			}
			else
	...
	UIViewLabels getContextMenuLabels() {
        ...
        labels.setAddLabel(OpenXDataFacade.getOpenXDataConstants().label_addnewsettinggroup());
        labels.setDeleteLabel(OpenXDataFacade.getOpenXDataConstants().label_deletesetting());
        labels.setAddChildItemLabel(OpenXDataFacade.getOpenXDataConstants().label_addnewsetting());
        return labels;
	}
	...
	@Override
	public void updateSettingGroups(OpenXDataObservable observable,	List<SettingGroup> settingGroups) {
		this.settingGroups = settingGroups;
