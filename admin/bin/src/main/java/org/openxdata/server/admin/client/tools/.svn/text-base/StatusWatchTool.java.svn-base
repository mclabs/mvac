...
/**
 * Shows System Status on the Diagnostic Tool.
 * 
 * @author Angel
 *
 */
public class StatusWatchTool extends OpenXDataBaseObserverCompositeView {
	...
	// Caption Label for latest Risk Level.
	private Label riskCriticalLevel;
	...
	// Display area for Risk level.
	private ListBox riskCriticalLevelBox;
	
	/**
	 * Constructs an instance of this type.
	 */
	public StatusWatchTool(){
	...
	/**
	 * Initializes the <tt>Widget</tt> with the relevant views.
	 */
	private void setupWidgets() {
		
		// Label properties
		latestError = new Label("Latest Error: ");
		riskCriticalLevel = new Label("Risk Level: ");
		...
		// Setting properties for Risk Level List Box.
		riskCriticalLevelBox = new ListBox();
		riskCriticalLevelBox.setWidth("200px");
	...
	/**
	 * Sets Up Event Listeners for this <tt>Widget.</tt>
	 */
	...
	public void update(OpenXDataObservable observable,	Object diagnosis) {
		...
		if(diagnosis != null){
			...
			riskCriticalLevelBox.addItem(((Diagnosis) diagnosis).getRiskLevel().valueOf(((Diagnosis) diagnosis).getRiskLevel()));
