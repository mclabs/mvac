...
/**
 * This widget displays properties of the selected setting and lets you edit them.
 * 
 * @author daniel
 * @author Angel
 *
...
public class SettingView extends OpenXDataBaseView {
	 ...
	 * 
	 * @param itemChangeListener listener to <tt>Setting</tt> property changes.
	 * @param openXDataViewFactory 
	 */
	public SettingView(ItemChangeListener itemChangeListener, OpenXDataViewFactory openXDataViewFactory){
	...
	protected void setUp() {
		...
		lblValue = new Label(OpenXDataFacade.getOpenXDataConstants().label_value());
	...
	private void loadView() {
		...
		Label nameLabel = new Label(OpenXDataFacade.getOpenXDataConstants().label_name());
		Label descLabel = new Label(OpenXDataFacade.getOpenXDataConstants().label_description());
	...
	private void loadPermissionLessView() {
		table.setWidget(0, 0, new Label(OpenXDataFacade.getOpenXDataConstants().ascertain_permissionLessView() + "Settings"));
	...
	private void updateName(){
		if(setting != null){
			...
			itemChangeListener.onItemChanged(setting);
		}
		else{
			settingGroup.setName(txtName.getText());
			itemChangeListener.onItemChanged(settingGroup);
		}
	}
