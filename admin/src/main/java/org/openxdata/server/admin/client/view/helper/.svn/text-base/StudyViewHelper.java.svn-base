...
public class StudyViewHelper {
	...
	public static void loadPermissionLessView(FlexTable table, DecoratedTabPanel tabs ) {
		table.setWidget(0, 0, new Label(OpenXDataFacade.getOpenXDataConstants().ascertain_permissionLessView() + "Studies"));
	...
	public static void saveExistingForm(String xml, String xformXml, FormData formData){
		try{
			...
			Context.getStudyManagerService().saveFormData(formData, new OpenXDataAsyncCallback<Void>() {
				@Override
