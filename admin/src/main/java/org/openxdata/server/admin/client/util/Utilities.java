...
public class Utilities {
	 ...
	 * Maximizes a widget.
	 * 
	 * @param widget the widget to maximise.
	 */
	 ...
	 * 
	 * @param textBox the text box widget.
	 * @param allowDecimal set to true to allow decimal points.
	 */
	 ...
	 * 
	 * @param tree the tree widget.
	 * @param item the root tree item to remove.
	 */
	...
	/**
	 * Gets the value of the description template from an xforms instance data node.
	 * 
	 * @param node the instance data node.
	 * @param template the description template.
	 * @return the description template value.
	 */
	private static String getDescriptionTemplate(Element dataNode, String template){
...
//		String s = "Where does ${name}$ come from?";
		...
		do{
			...
			v = getValue(dataNode,text.substring(i+2, j)); //append value of token.
	 ...
	 * 
	 * @param xformXml, the xforms xml. This should not be the data xml since it may have been
	 * 				created and saved without the description template.
	 * 
	 * @return the description template value.
	 */
	public static String getDescriptionTemplate(String xformXml, String dataXml){
		Element node = XformUtil.getInstanceDataNode(XmlUtil.getDocument(xformXml));
		if(node == null)
			return null;
		String descTemplate = node.getAttribute("description-template");
		return getDescriptionTemplate(XmlUtil.getDocument(dataXml).getDocumentElement(),descTemplate);
	}
	...
	/**
	   ...
	   */
	public static native void disableContextMenu(com.google.gwt.user.client.Element elem) /*-{
	...
	public static boolean validateUserPassword(User user){
		...
		if(user != null){
			...
			if(user.isNew() 
					&& user.getClearTextPassword() != null){
	...
	public static void displayMessage(String message){
		OpenXDataMessageDialog messageDialog = new OpenXDataMessageDialog("System Notification");
	 ...
	 * 
	 * @param tree <tt>Tree View</tt> to select <tt>item</tt> on.
	 */
	...
	/**
	 * Displays notification Messages one after another with a time lag of 5 seconds.
	 * 
	 * @param firstMessage First Message to display.
	 * @param secondMessage Second Message to display.
	 * @param lastMessage Last Message flag to indicate completion of operation.
	 */
	public static void displayNotificationMessageAsynchronously(String firstMessage, final String secondMessage, String lastMessage){
	...
	/**
	 * Displays an <tt>Error Message</tt> on the <tt>Notification Bar</tt> with appropriate formatting.
	 * 
	 * @param failureMessage <tt>Message</tt> to display.
	 */
	public static void displayFailureNotificationMessage(String failureMessage) {		
		//Display the failure Message
		((OpenXDataLabel) widgetFactory.getNotificationLabel()).setFailureText(failureMessage);
	 ...
	 * <p>
	 * The simplest validation (99% of email address). For example does not allow for IP address emails which are rarely used.
	 * </p>
	 * @param emailAddress Email/Emails to validate. Assumes they are separated with ','.
	...
	public static boolean validateEmail(String emailAddress) {
		...
		if(emailAddress.contains(",")){
			...
			if (emailAddress != null && !emailAddress.equals("")) {
				validated = emailAddress.trim().matches("^[\\w-]+(\\.[\\w-]+)*@(?:[\\w-]+\\.)+[a-zA-Z]{2,7}$");
			}
		}
		return validated;
	}
	 ...
	 * 
	 * @param string <tt>String</tt> to check.
	 ...
	 * 
	 * @param username <tt>User name</tt> we checking.
	 * 
