...
public class XformUtil {
	...
	public static String addFormId2Xform(Integer versionId,String xml) {
		...
		try {
			...
			doc = db.parse(IOUtils.toInputStream(xml,"UTF-8"));
			
			NodeList elemList = doc.getDocumentElement().getElementsByTagName("xf:instance"); //TODO Do not hardcode the node prefix
			if (elemList != null && elemList.getLength() > 0) {
				...
				for(int index = 0; index < kids.getLength(); index++){
					Node node = kids.item(index);
					if(node.getNodeType() == Node.ELEMENT_NODE){
						((Element)node).setAttribute(OpenXDataConstants.ATTRIBUTE_NAME_FORMID, versionId.toString());
						break;
					}
				}
			}
