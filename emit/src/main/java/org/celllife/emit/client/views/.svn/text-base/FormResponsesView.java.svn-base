...
public class FormResponsesView extends View implements Refreshable  {
    ...
    protected void initialize() {
        ...
        Log.debug("FormResponsesView : initialize");
        window = new Window();
			...
			public void handleEvent(ButtonEvent be) {
                DeferredCommand.addCommand(new Command() {
					...
					public void execute() {
                        ...
                        Log.debug("Loading CSV from URL "+url);
    ...
    private void initializeColumnModel() {
    	Log.debug("FormResponsesView : initializeColumnModel");
    ...
    private void preInitialise(ListStore<FormDataSummary> store, ColumnModel columnModel) {
        Log.debug("FormResponsesView : preInitialise");
    ...
    private void initializeGrid() {
    	Log.info("FormResponsesView : initializeGrid");
    ...
    private void initializePagingLoader() {
    	Log.debug("FormResponsesView : initializePagingLoader");
        ...
        loader = new BasePagingLoader<PagingLoadResult<ModelData>>(
                new RpcProxy<PagingLoadResult<FormDataSummary>>() {
                    ...
                    public void load(Object loadConfig, final AsyncCallback<PagingLoadResult<FormDataSummary>> callback) {
                        Log.debug("FormResponsesView RpcProxy:load grid.isViewReady="+grid.isViewReady());
    ...
    private void initializePagingToolbar() {
    	Log.debug("FormResponsesView : initializePagingToolbar");
			...
			public void selectionChanged(SelectionChangedEvent<SimpleComboValue<String>> se) {
                ...
                if (pageSizeText.length > 0) {
                    try {
                        ...
                        Log.error("Page size "+pageSizeText[0]+" is not a number", e);
                    }
                } else {
                    Log.error("Page size could not be found in the selected text " + se.getSelectedItem().getValue());
                }
            }
        });
    ...
    private void initializeColumnEditor(ColumnConfig colConfig, QuestionDef questionDef) {
    	Log.debug("FormResponsesView : initializeColumnEditor");
        ...
        */
        ...
        switch(questionDef.getDataType()) {
            ...
            case QuestionDef.QTN_TYPE_DATE:
                ...
                colConfig.setDateTimeFormat(DateTimeFormat.getMediumDateFormat());
                break;
            case QuestionDef.QTN_TYPE_TIME:
                TimeField time = new TimeField();
                time.setEditable(false); // avoids them entering invalid data
                time.setFormat(DateTimeFormat.getShortTimeFormat());
                colConfig.setDateTimeFormat(DateTimeFormat.getShortTimeFormat());
    ...
    protected void handleEvent(AppEvent event) {
    	Log.debug("FormResponsesView : handleEvent");

        if (event.getType() == FormResponsesController.BROWSE) {
             Log.debug("handling browse event "+System.currentTimeMillis());
             ProgressIndicator.showProgressBar();
    ...
    private void initializeWindow(String title, Component component, Component bottomComponent) {
    	Log.debug("FormResponsesView : createWindow");
        window = new Window();
	...
	public void refresh(RefreshableEvent event) {
    	Log.info("FormResponseView : refresh response view");
    	toolBar.refresh();
	} 
	...
	public void setUser(User user) {
        ...
        if (!user.hasPermission("Perm_Data_Edit")) {
        	rowEditor.disable();
        	Log.info("user does not have permission to edit data, so roweditor was disabled");
        }
	}
}