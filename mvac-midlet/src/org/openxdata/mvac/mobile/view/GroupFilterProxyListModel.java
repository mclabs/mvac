 ...
 * @author soyfactor
 */
public class GroupFilterProxyListModel implements ListModel, DataChangedListener {
   private ListModel underlying;
   private Vector filter;
   private Vector listeners = new Vector();
   public GroupFilterProxyListModel(ListModel underlying) {
   ...
   public void filter(String str) {
       ...
       for(int iter = 0 ; iter < underlying.getSize() ; iter++) {
           String element = ((AppointmentWrapper)underlying.getItemAt(iter)).getName();
